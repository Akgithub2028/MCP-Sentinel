...
from mcp_security_common.hash_utils import compute_tool_hash, create_tool_pin
from mcp_security_common.mcp_types import MCPTool, ServerPinStore, ToolPin


class SchemaPinStore:
    def __init__(self, pin_file_path: Optional[Path | str] = None):
    ...
    def load(self) -> None:
        if not self.pin_file_path or not self.pin_file_path.exists():
            return
        with open(self.pin_file_path, "r", encoding="utf-8") as f:
            data = json.load(f)
    ...
    def verify_tool(self, tool: MCPTool) -> Tuple[bool, Optional[str], Optional[str]]:
        """
